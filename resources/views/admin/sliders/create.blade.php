    ...
    <div class="col-12 col-md-9">
        <div class="card">
            ...
            <form action="{{ route('admin.sliders.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <x-form-inputfile label="Slider Resmi (Web)" name="image"/>
                    <x-form-inputfile label="Slider Resmi (Mobil)" name="imagemobil"/>
                            ...
                            <div class="tab-content">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                    <div class="tab-pane @if($loop->first) active @endif" id="{{ $localeCode }}">
                                        <x-form-inputtext label="Başlık" name="{{ $localeCode }}[title]"/>
                                        <x-form-inputtext label="Link" name="{{ $localeCode }}[link]"/>
                                        <x-form-inputtext label="Link Yazı" name="{{ $localeCode }}[link_text]"/>
                                        <x-form-inputtext label="Yazı 1" name="{{ $localeCode }}[text1]"/>
                                        <x-form-inputtext label="Yazı 2" name="{{ $localeCode }}[text2]"/>
                                        <x-form-inputtext label="Yazı 3" name="{{ $localeCode }}[text3]"/>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-between">
                            <x-form-switch name="status" label="Durum" />
